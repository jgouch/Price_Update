...
def surgical_update(inv_path, master_path, output_path):
    ...
    for ws in wb.worksheets:
        ...
        for row in ws.iter_rows(min_row=header_row, max_row=ws.max_row):
            ...
            # Formatting Loop
            for cell in row:
                # UNMERGE Safety Check: Only skip if NOT writing data.
                # If we are writing data, we handle the unmerge below.
                # Here we just want to avoid formatting errors on merged children.
                    ...
                    if new_pct is not None and new_pct != "N/A":
                        for c, name in col_indices.items():
                            if any(k in name for k in ['%', 'SOLD', 'SCARCITY']) and 'QTY' not in name: 
                                ...
                                # Force Unmerge if needed
                                if isinstance(cell, MergedCell):
                                    for merged_range in ws.merged_cells.ranges:
                                        if cell.coordinate in merged_range:
                                            ws.unmerge_cells(str(merged_range))
                                            break
                                    cell = ws.cell(row=r_idx, column=c) # Get fresh cell
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 update_v34.py [File1] [File2]")
    else:
