...
# --- 2. HEADER HUNTER ---
def find_true_header_row(file_path):
    """
    Scans the first 20 rows of the Excel file to find the row
    that actually contains the column headers (Garden, Status, etc).
    """
    # Read just the top 20 rows first (no header assumption yet)
    ...
    for i, row in df_temp.iterrows():
        ...
        # If we find both 'GARDEN' and 'STATUS' (or similar), this is the header row
        if any('GARDEN' in v for v in row_values) and any('STATUS' in v for v in row_values):
            return i
            
    return 0 # Default to 0 if not found (fallback)

# --- 3. SMART COLUMN DETECTIVE ---
def identify_columns(df):
    ...
    # FIND ROW/SECTION
    # Priority: Section -> Row -> Lot -> Block
    candidates = [c for c in cols if any(x in c.upper() for x in ['SECTION', 'ROW', 'BLOCK', 'LOT', 'TIER'])]
    # Pick the best one (usually 'Section' or 'Row')
    if candidates:
        # Prefer 'Section' or 'Row' if available
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 update_inventory_v4.py [Inventory_File] [Master_Price_Book]")
        return
    ...
    print(f"\nScanning Inventory File: {os.path.basename(inv_path)}...")

    try:
        # STEP 1: Find the true header row
        header_row_idx = find_true_header_row(inv_path)
        print(f"✅ Found Header at Row {header_row_idx + 1}")

        # STEP 2: Read file starting from that row
        df_inv = pd.read_excel(inv_path, header=header_row_idx)
        ...
        if None in col_map.values():
            print("\n❌ CRITICAL ERROR: Could not identify columns even after finding header.")
            print(f"Columns Found: {list(df_inv.columns)}")
            return
