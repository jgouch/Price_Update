...
def calculate_percent_sold(df_inventory, garden_name_full, col_map):
    ...
    if target_garden == "GRACE" and col_section:
        ...
        if "SIDEWALK" in garden_name_full.upper():
            ...
            garden_data = garden_data[~is_sidewalk]
            # Debug Print for Standard Grace
            # print(f"   >>> DEBUG: Calculating Grace Standard. Total Rows: {len(garden_data)}")
    ...
    result = (total - len(garden_data[avail_mask])) / total
    return result
...
def surgical_update(inv_path, master_path, output_path):
    ...
    for ws in wb.worksheets:
        ...
        for row in ws.iter_rows(min_row=header_row, max_row=ws.max_row):
            ...
            if not is_header:
                if row_col_idx: row_name = str(ws.cell(row=r_idx, column=row_col_idx).value).strip()
                if garden_col_idx: garden_name = str(ws.cell(row=r_idx, column=garden_col_idx).value).strip()
            ...
            # --- DATA UPDATE (WITH BRUTE FORCE FOR GRACE) ---
            if not is_header:
                if garden_name:
                    new_pct = calculate_percent_sold(df_inv, garden_name, col_map)
                    
                    # GRACE STANDARD DEBUG & FORCED WRITE
                    if "Grace (standard" in garden_name:
                        print(f"   >>> TARGET ACQUIRED: 'Grace (standard)'. Calculated: {new_pct}")
                        if new_pct is not None:
                            # Force write to Col 2 (B) if no header match found
                            found_col = False
                            for c, name in col_indices.items():
                                if any(k in name for k in ['%', 'SOLD', 'SCARCITY']) and 'QTY' not in name:
                                    ...
                                    cell.number_format = '0%'
                                    cell.data_type = 'n'
                                    print(f"   >>> SUCCESS: Wrote {new_pct} to {cell.coordinate}")
                            
                            # FALLBACK: If loop finished and nothing wrote, FORCE Column 2
                            if not found_col:
                                print(f"   >>> WARNING: Could not find % column. FORCING write to Column 2.")
                                cell = ws.cell(row=r_idx, column=2)
                                cell.value = new_pct
                                cell.number_format = '0%'
                                cell.data_type = 'n'

                    # Normal Write for others
                    elif new_pct is not None and new_pct != "N/A":
                        for c, name in col_indices.items():
                            if any(k in name for k in ['%', 'SOLD', 'SCARCITY']) and 'QTY' not in name: 
                                cell = ws.cell(row=r_idx, column=c)
                                if isinstance(cell, MergedCell):
            ...
            for cell in col:
                if cell.row >= header_row and cell.value and not isinstance(cell, MergedCell):
                    try: 
                        ...
                        if val_len > 0:
                             ws.column_dimensions[col_letter].width = min(max(ws.column_dimensions[col_letter].width, (val_len + 2) * 1.1), 50)
                    except: pass
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 update_v35.py [File1] [File2]")
    else:
