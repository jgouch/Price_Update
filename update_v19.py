...
def calculate_percent_sold(df_inventory, garden_name_full, col_map):
    ...
    # 2. CLEAN PARENTHESES from Main Garden Name
    # Turns "Grace (standard spaces)" -> "Grace"
    main_garden = re.sub(r'\(.*?\)', '', main_garden).strip()
...
def surgical_update(inv_path, master_path, output_path):
    print("⚙️  Starting Update (Fixing Missing %)...")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 update_v19.py [File1] [File2]")
    else:
