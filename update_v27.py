...
# --- 5. CALCULATIONS (WITH GRACE LOGIC) ---
def is_grace_sidewalk(space_str):
    """
    Checks if the space belongs to the Sidewalk/Roadside sections of Grace.
    Sections: 30, 40, 50, 60-64, 70-74, 80, 90, 100, 110, 120.
    Expects string like: "Garden of Grace Lot/Section 30 Space 1"
    """
    if not isinstance(space_str, str): return False
    
    # Extract the number after "Lot/Section" or just grab digits
    # Regex looks for "Lot/Section" followed by digits
...
def calculate_percent_sold(df_inventory, garden_name_full, col_map):
    ...
    if target_garden == "GRACE":
        ...
        if "SIDEWALK" in garden_name_full.upper():
            ...
            print(f"   > Grace (Sidewalk): Found {len(garden_data)} inventory rows.")
        elif "STANDARD" in garden_name_full.upper():
            garden_data = garden_data[~is_sidewalk] # The inverse (Not sidewalk)
            print(f"   > Grace (Standard): Found {len(garden_data)} inventory rows.")
        elif "INFANT" in garden_name_full.upper():
            return "N/A" # Skip Infant
        elif "MATTHEW" in garden_name_full.upper():
            # If no rule for Matthews, return None or handle if you have a rule
            return None 
...
def surgical_update(inv_path, master_path, output_path):
    ...
    for ws in wb.worksheets:
        ...
        for row in ws.iter_rows(min_row=header_row, max_row=ws.max_row):
            ...
            for cell in row:
                if isinstance(cell, MergedCell): continue # Skip merged cells
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 update_v27.py [File1] [File2]")
    else:
