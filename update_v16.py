        ...
        for row in ws.iter_rows(min_row=header_row, max_row=ws.max_row):
            ...
            for cell in row:
                ...
                if is_header:
                    ...
                    cell.font = Font(name=FONT_NAME, size=10, color="000000") # Force Black Text
                    ...
                    # 1. CURRENCY (Price, Total, Cost, Front, Plaque, Crypt)
                    if any(x in col_name for x in ['PRICE', 'TOTAL', 'RIGHT', 'COST', 'PLAQUE', 'ETCHING', 'FRONT', 'CRYPT', 'NICHE', 'MEMORIAL', 'BURIAL', 'OPTION']):
                        # Only format as money if it's actually a number column
                        # (Avoids formatting "Single/Companion" text columns as money)
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 update_v16.py [File1] [File2]")
        return
