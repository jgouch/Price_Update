...
def calculate_percent_sold(df_inventory, garden_name_full, col_map):
    ...
    # regex=False ensures literal matching
...
def surgical_update(inv_path, master_path, output_path):
    print("⚙️  Starting Scarcity Verification & Update...")
    ...
    for ws in wb.worksheets:
        ...
        for row in ws.iter_rows(min_row=header_row, max_row=ws.max_row):
            ...
            # Check for stacked sections (like "Lakeside Tower - West")
            for cell in row:
            ...
            for cell in row:
                ...
                if is_header:
                    ...
                    if any(x in col_name for x in ['PRICE', 'TOTAL', 'RIGHT', 'COST', 'PLAQUE', 'ETCHING', 'FRONT', 'CRYPT', 'NICHE', 'MEMORIAL', 'BURIAL', 'OPTION']):
                        ...
                        if val_str not in ['SINGLE', 'COMPANION', 'TANDEM', 'SIDE-BY-SIDE', 'NONE']:
                    ...
                    # 3. AVAILABILITY / COUNTS (Scarcity Check)
                        ...
                        if val_str == "SOLD OUT":
                            ...
                            # SCARCITY HIGHLIGHT: If less than 4 left, mark Orange/Bold
                            if 0 < cell.value < 4:
                                cell.font = Font(name=FONT_NAME, size=10, color="E26B0A", bold=True) # Orange
        ...
        # 3. AUTO-FIT COLUMNS (FIXED FOR MERGED CELLS)
        for i, col in enumerate(ws.columns, 1):
            max_len = 0
            # Get safe column letter
            col_letter = get_column_letter(i)
            
            for cell in col:
                # Only check width for rows below the title (approx row 4+)
                # This prevents huge titles from making columns too wide
                if cell.row >= header_row and cell.value:
                    try: max_len = max(max_len, len(str(cell.value)))
                    except: pass
            
            # Apply width if data exists
            if max_len > 0:
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 update_v17.py [File1] [File2]")
        return
